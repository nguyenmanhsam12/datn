...
.custom-btn:hover {
    opacity: 0.7;  /* Khi hover sẽ mờ đi */
}

.custom-btn i {
    font-size: 1.5rem;  /* Điều chỉnh kích thước icon */
    transition: transform 0.3s ease;  /* Hiệu ứng cho icon */
}
...
.modal-dialog {
    ...
    margin-top: 5%; /* Để modal không quá sát với phía trên */
}

/* Điều chỉnh nội dung modal */
.modal-body {
    max-height: 400px; /* Giới hạn chiều cao của nội dung */
    overflow-y: auto; /* Nếu nội dung vượt quá chiều cao, thanh cuộn sẽ xuất hiện */
}
    ...
    <table class="table mt-3">
        ...
        <tbody>
            @foreach ($posts as $post)
                <tr> 
                    <td>
                        <button type="button" class="btn custom-btn" data-toggle="modal" data-target="#productModal{{ $post->id }}">
                            <i class="fas fa-exclamation-circle"></i>
                        </button>
                    </td>
                    <!-- CHi tiết -->
                    <div class="modal fade" id="productModal{{ $post->id }}" tabindex="-1" role="dialog" aria-labelledby="productModalLabel{{ $post->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title post-title" id="productModalLabel{{ $post->id }}"> {{ $post->title }}</h5>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-4">
                                            <img src="{{ asset($post->thumbnail) }}" alt="Thumbnail" class="img-fluid" style="width: 200px; height: 300px;">
                                        </div>
                                        <div class="col-md-8">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4">
                                            @if($post->secondary_image)
                                                <img src="{{ asset($post->secondary_image) }}" alt="Secondary Image" class="img-fluid" style="width: 200px; height: 300px;">
                                            @endif
                                        </div>
                                        <div class="col-md-8">
...
{{-- end chi tiết --}}
                    <td>
                    @if($post->thumbnail)
                    <img src="{{ asset($post->thumbnail) }}" alt="Thumbnail" style="width: 80px; height: auto;">
                @else
